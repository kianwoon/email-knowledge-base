...
async def auth_callback(
    ...
    try:
        ...
        # --- CORRECTED USER HANDLING --- 
        # 1. Extract necessary info
        ms_graph_id = user_info.get("id")
        user_email = user_info.get("mail") or user_info.get("userPrincipalName")
        user_display_name = user_info.get("displayName")
        user_organization = user_info.get("officeLocation") # Optional

        if not user_email:
             raise ValueError("Could not determine user email from MS Graph info.")
        if not user_display_name:
              user_display_name = user_email # Fallback display name
              
        # 2. Get or Create User in DB using email (CRUD function handles internal UUID)
        # Assumes a CRUD function exists that:
        # - Finds user by email.
        # - If not found, creates user with a new internal UUID, stores ms_graph_id.
        # - Updates display_name, photo_url, organization, last_login.
        # - Returns the SQLAlchemy UserDB object.
        db_user = user_crud.get_or_create_user_from_ms_graph(
            db=db, 
            email=user_email, 
            ms_id=ms_graph_id, 
        ...
        logger.info(f"User {db_user.email} retrieved/created in database with internal ID: {db_user.id}")
        
        # Update last login time (might also be handled in CRUD)
        db_user.last_login = datetime.now(timezone.utc)
        # --- END CORRECTED USER HANDLING ---

        # --- Store MS Refresh Token in DB (using db_user) ---
        if ms_refresh_token:
            ...
            if encrypted_token_bytes:
                ...
                # Store Access Token and Expiry using db_user
                if ms_access_token:
                    db_user.ms_access_token = ms_access_token
        ...
        # --- Create JWT using internal ID from db_user ---
        jwt_subject = db_user.email
        # Use the internal UUID from the DB object
        jwt_user_id = str(db_user.id) 
        ...
        jwt_token, expires_at = AuthService.create_user_jwt(
            user_id=jwt_user_id, # Pass internal UUID
            email=jwt_subject, 
            # ms_token no longer needed in JWT payload
